...
- **分享长图的水印**（默认只在便签应用里生效）：便签自己「分享为图片」时底部那行
  ColorOS 水印，可选 **去掉（默认）/ 去掉文字但保留原高度（留白）/ 换成我自己的文字 / 不动它**。
  水印是应用自己布局的一部分，只能在它画图的过程中处理，所以这一项是 hook 而不是后期裁剪；
  自定义文字保存在设置里，切回该模式仍然在。
  **它为什么不进导出的长图**：模块导出时取的是编辑器自己的页位图，而水印属于应用"分享卡片"那一层
  ——卡片、边距、水印都不经过我们的链路（这正是导出产物没有 ColorOS 水印的原因）。
  想让水印文字出现在导出里，见下一条"便签式卡片版"
- **便签式卡片版**（可选，默认关闭）：打开后导出改成像便签"分享为图片"那样的卡片 —— 宽 1094 px
  （编辑器原始是 1264）、四周卡面边距，底部按上面那一项画一条分割线加你的文字（选「去掉」或
  「不动它」则不画，「留白」留一条空白）。卡片由模块自己合成，字体与间距是仿的，不会与便签
  自己分享的那张逐像素一致；好处是**水印文字终于能出现在导出产物里**，而且不依赖任何 hook
- **选择要导出哪些便签**：设置页的「选择便签…」会向便签应用要来一份清单（标题 · 分类 · 字数，
  加密与回收站中的便签会标出来），勾选后只导出这些；**一条都不勾 = 全部**。选择会被记住，
  下次导出同一批不用重新勾。列表可以用搜索框按标题/分类过滤，也可以按分类筛选；
  「全选 / 全不选」只作用于当前显示出来的那几条
- **先试 1 条 + 预览**：先只跑第 1 条（不记住条数），导出的第一张图的**顶部**会显示在设置页上，
  用来核对底色、字色与版式——整张长图太长，只留顶部一截，否则一个 binder 回包放不下
- **同时导出原始附件**（默认关闭）：打开后每条便签的附件会另外拷一份到同名的「_附件」目录。
  默认关闭是因为长图里本来就已经有那些照片；便签应用自己的占位图（照片还没下载时的灰框）
  无论开关都不会导出。Word 文档不受影响，它需要嵌入的图照常嵌入
...
- **整本备份（zip）**：设置页一个按钮，把便签应用的 `databases/`、`files/`、`shared_prefs/`、
  `no_backup/` 打包成一个 zip 放进导出目录。**不需要 root**——模块本来就运行在便签进程里，
  那是唯一能读它自己数据目录的地方。数据库用 SQLite 的 `VACUUM INTO` 取一致性快照（应用正在用这个库，
  直接拷文件可能拷到打不开的库），附件按原样流式写进 zip。**不提供恢复功能**：恢复要 root 或 adb，
  包内「备份说明.txt」里写着每个目标路径与逐条命令（`/data/data/com.coloros.note/databases/`、
  `.../files/<便签 id>/` 等），设置页也把这两个目录直接写在按钮上方
- **导出进度条 + 取消导出**：导出跑在便签进程里，模块自己的进程看不到，所以便签那一侧把进度
  （第几条 / 共几条 / 正在画哪一条）与"是否请求取消"放在它自己的内存里，设置页每 0.7 秒问一次
  并画进度条，顺带按已完成的平均耗时给出「（约剩 N 秒/分）」；「取消导出」是请求而不是硬停——
  正在画的那一条会画完，从下一条起不再处理
- **通知栏进度**：三种格式都会发通知（Word 与模块绘制以前没有），导出结束后通知上多一个
  「打开导出目录」的动作，点一下直接跳到产物目录
...
src/com/qkqwork/noteexport/
  ...
  NoteCard.java              便签式卡片版：卡面边距 + 底部按水印设置画的分割线与文字
